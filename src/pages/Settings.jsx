...
export default function Settings() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                if (mounted) { setViewData(base); setEditData(base); }
            }
        })();
    ...
    const onBizFileChange = async (e) => {
        ...
        if (!f) return;
        if (!companyId) { emitToast("회사 정보를 찾을 수 없습니다.", "error"); return; }
        ...
        try {
            ...
            setViewData((prev) => ({ ...prev, bizCertDocGcsObjectName: objectName, bizCertDocName: f.name }));
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const objName = viewData?.bizCertDocGcsObjectName || (Array.isArray(viewData?.bizCertDocGcsObjectNames) && viewData.bizCertDocGcsObjectNames[0]) || "";
                if (!objName) {
                    if (!cancelled) { setBizPreviewUrl(""); setBizPreviewKind(""); }
                    return;
                }
    ...
    const handlePointsChange = useCallback((newPoints, _idxInMap) => {
            ...
            <div className="settings-container">
                ...
                <div className="settings-left-panel">
                    ...
                    <div className="settings-card card-company-profile">
                        ...
                        <div className="company-logo-area" onClick={() => fileInputRef.current?.click()} style={{cursor: 'pointer'}}>
                            {companyInfo?.logoPath ? (
                                <GCSImage objectName={companyInfo.logoPath} alt="Company Logo" className="company-logo-img" />
                            ) : companyInfo?.logoDataUrl ? (
                                <img src={companyInfo.logoDataUrl} alt="Company Logo" className="company-logo-img" />
                            ) : (
                    ...
                    <div className="settings-card card-biz-cert">
                        <div className="biz-cert-preview-area" onClick={() => bizPreviewUrl && setBizViewerOpen(true)}>
                            {bizPreviewUrl && (bizPreviewKind === 'image' || bizPreviewKind === 'pdf') ? (
                                bizPreviewKind === 'image' ? (
                                    ...
                                    <div style={{display:'flex', alignItems:'center', justifyContent:'center', height:'100%', flexDirection:'column'}}>
                                        <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#888" strokeWidth="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline></svg>
                                        <span style={{fontSize:'12px', color:'#888', marginTop: 8}}>PDF 문서</span>
                                    </div>
                                )
                            ) : (
                    ...
                    <div className="settings-card card-opt-out">
                        ...
                        <div className="opt-out-desc"> {/* Updated description */}
                ...
                <div className="settings-right-panel">
                        ...
                        {isAddingNew ? (
                            ...
                            <KakaoMap
                                polygons={mapPolygons.map(item => ({ 
                                    ...
                                    name: item.name,
                                    options: {
                                        fillColor: selectedGeofenceIdx !== null && displayItems[selectedGeofenceIdx] === item ? '#E2F1FE' : '#A2D5F2',
                                        strokeColor: selectedGeofenceIdx !== null && displayItems[selectedGeofenceIdx] === item ? '#006CEC' : '#006CEC',
                                        strokeWeight: 2
                                    }
                                }))}
                                height="100%"
                                ...
                                center={selectedGeofenceIdx !== null && displayItems[selectedGeofenceIdx] && displayItems[selectedGeofenceIdx].points[0] ? displayItems[selectedGeofenceIdx].points[0] : undefined}
                    ...
                    <div className="geofence-list-overlay">
                        <div className="geofence-list-header">
                            ...
                            <button className="geofence-add-btn" onClick={() => { setIsAddingNew(!isAddingNew); setSelectedGeofenceIdx(null); }}>
                                {isAddingNew ? (
                        ...
                        {isAddingNew && (
                            <div className="geofence-edit-form">
                                ...
                                <div className="geofence-actions">
                                    <button className="biz-cert-upload-btn" style={{width:'auto', flex:1}} onClick={handleNewGeofenceSave}>저장</button>
                                </div>
                            </div>
                        )}
                            ...
                            {displayItems.length === 0 ? (
                                ...
                                displayItems.map((item, idx) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            {isEditing ? (
                                                <div className="geofence-edit-inline" onClick={(e) => e.stopPropagation()}>
                                                    <input 
                                                        className="geofence-input" 
                                                        value={item.name} 
