...
function MemberManagement() {
  ...
  const pendingRows = useMemo(
    () => pendingMembers.map((m) => ({ ...m, id: m.userId })),
    [pendingMembers]
  );

  // Build unified rows: pending first, then approved, then withdrawn
  const unifiedRows = useMemo(() => {
    const mapAll = allRows.map((m) => ({
      ...m,
      _pending: false,
      displayRole: m.role || ROLES.MEMBER,
    }));
  ...
  const columns = useMemo(() => {
    const cols = [
      ...
      },
      {
        ...
        render: (r) => (
          <span className={`role-badge role-${r.displayRole}`}>{r.displayRole || 'member'}</span>
        ),
      },
      {
        key: 'status',
  ...
  const selectedPendingCount = useMemo(
    () => selection.selectedItems.filter((r) => r && r._pending).length,
    [selection.selectedItems]
  );
  const selectedWithdrawEligibleCount = useMemo(
    () =>
      selection.selectedItems.filter(
        (r) => r && !r._pending && r.membershipStatus !== 'withdrawn' && canManageMember(r)
      ).length,
    [selection.selectedItems]
  );
      ...
      if (isAdmin && user && user.companyId) {
        list = list.filter(
          (m) =>
            ...
            (m.role === ROLES.ADMIN || m.role === ROLES.MEMBER)
        );
      }
  ...
  async function handleRoleChange() {
    ...
    if (
      !window.confirm(
        `${selectedMember.userId}의 권한을 "${newRole}"로 변경하시겠습니까?\n\n대상 사용자는 재로그인이 필요합니다.`
      )
    ) {
      return;
    }
  ...
  async function ensureNotLastAdmin(target) {
    ...
    const adminCount = latest.filter(
      (m) =>
        m &&
        m.companyId === companyId &&
        m.role === ROLES.ADMIN &&
        m.membershipStatus !== 'withdrawn'
    ).length;
    if (adminCount <= 1) {
      const ok = await confirm({
        title: '마지막 관리자 경고',
        message:
          '해당 사용자는 해당 회사의 마지막 관리자입니다. 탈퇴 시 회사 관리 권한이 사라집니다. 계속 진행하시겠습니까?',
        confirmText: '계속',
        cancelText: '취소',
      });
      return !!ok;
  ...
  async function handleWithdraw(member) {
    ...
    const ok = await confirm({
      title: '회원 탈퇴',
      message: `${member.userId} 사용자를 탈퇴 처리하시겠습니까?`,
      confirmText: '탈퇴',
      cancelText: '취소',
    });
    if (!ok) return;
  ...
  async function handleRestore(member) {
    ...
    const ok = await confirm({
      title: '회원 복원',
      message: `${member.userId} 사용자를 복원하시겠습니까?`,
      confirmText: '복원',
      cancelText: '취소',
    });
    if (!ok) return;
  ...
  async function handleBulkApprove() {
    ...
    const targets = ids
      .map((id) => unifiedRows.find((r) => r.id === id))
      .filter((r) => r && r._pending && canManageMember(r));
    if (!targets.length) return emitToast('선택된 대기 회원이 없습니다.', 'warning');
    const ok = await confirm({
      title: '일괄 승인',
      message: `${targets.length}명의 대기 회원을 승인하시겠습니까?`,
      confirmText: '승인',
      cancelText: '취소',
    });
    if (!ok) return;
    try {
      ...
      emitToast(
        `승인 완료: ${successCount}/${targets.length}`,
        successCount === targets.length ? 'success' : successCount > 0 ? 'warning' : 'error'
      );
      await loadPendingMembers();
      await loadAllMembers();
  ...
  async function handleBulkReject() {
    ...
    const reason = window.prompt(
      `${targets.length}명의 대기 회원을 거절합니다. 사유를 입력하세요 (선택):`,
      ''
    );
    ...
    try {
      ...
      emitToast(
        `거절 완료: ${successCount}/${targets.length}`,
        successCount === targets.length ? 'success' : successCount > 0 ? 'warning' : 'error'
      );
      await loadPendingMembers();
      selection.clearSelection();
  ...
  async function handleBulkWithdraw() {
    ...
    const targets = ids
      .map((id) => unifiedRows.find((r) => r.id === id))
      .filter((r) => r && !r._pending && r.membershipStatus !== 'withdrawn' && canManageMember(r));
    if (!targets.length) return emitToast('선택된 회원이 없습니다.', 'warning');
    const ok = await confirm({
      title: '일괄 탈퇴',
      message: `${targets.length}명의 회원을 탈퇴 처리하시겠습니까?`,
      confirmText: '탈퇴',
      cancelText: '취소',
    });
    if (!ok) return;
    try {
      ...
      emitToast(
        `탈퇴 처리 완료: ${successCount}/${targets.length}`,
        successCount === targets.length ? 'success' : successCount > 0 ? 'warning' : 'error'
      );
      await loadAllMembers();
      selection.clearSelection();
    ...
    return (
      <div className="page page--data member-management-page space-y-4">
        <div className="page-scroll space-y-4">
          <div className="error-message">접근 권한이 없습니다. 관리자 권한이 필요합니다.</div>
        </div>
      </div>
    );
  }
