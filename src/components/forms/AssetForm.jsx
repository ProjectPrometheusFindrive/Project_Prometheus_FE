...
export default function AssetForm({ initial = {}, readOnly = false, onSubmit, formId, showSubmit = true }) {
  ...
  return (
    <form id={formId} className="form-grid" onSubmit={handleSubmit}>
      ...
      <input id="vehicleType" className="form-input" value={form.vehicleType} onChange={(e) => update("vehicleType", e.target.value)} placeholder="예: 쏘나타 25년형" required disabled={readOnly} />

      <label className="form-label" htmlFor="registrationDate">차량등록일</label>
      ...
      <label className="form-label" htmlFor="registrationDoc">자동차등록증 첨부</label>
      ...
      <label className="form-label" htmlFor="insuranceDoc">보험가입증명서 첨부</label>
