...
export default function RentalForm({ initial = {}, readOnly = false, onSubmit, formId, showSubmit = true }) {
  ...
  return (
    <form id={formId} className="form-grid" onSubmit={handleSubmit}>
      ...
      <label className="form-label" htmlFor="renter_name">계약자 이름</label>
      ...
      <label className="form-label" htmlFor="contact_number">계약자 연락처</label>
      <input id="contact_number" className="form-input" value={form.contact_number} onChange={(e) => update("contact_number", e.target.value)} placeholder="예: 010-1234-5678" disabled={readOnly} />

      <label className="form-label" htmlFor="address">계약자 주소</label>
      ...
      <label className="form-label" htmlFor="start">대여 시작일</label>
      ...
      <label className="form-label" htmlFor="end">대여 종료일</label>
      ...
      <label className="form-label" htmlFor="insurance_name">보험사</label>
      ...
      {/* 대여 계약서 업로드 */}
      <label className="form-label" htmlFor="contract_file">대여 계약서 업로드</label>
      <input
        id="contract_file"
        type="file"
      ...
      {/* 운전면허증 업로드 */}
      <label className="form-label" htmlFor="driver_license_file">운전면허증 업로드</label>
      <input
        ...
        disabled={readOnly}
      />
      {form.driver_license_file && (
        <div style={{ fontSize: 12, color: "#555" }}>{form.driver_license_file.name}</div>
      )}
